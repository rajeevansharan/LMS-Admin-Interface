...
const QuizTimer: React.FC<QuizTimerProps> = ({ onTimeUp }) => {
  ...
  useEffect(() => {
    console.log(
      `[QuizTimer] Starting timer with 30 minutes (${30 * 60} seconds)`,
    );
    ...
    const timerId = setInterval(() => {
      setTimeLeft((prevTime) => {
        ...
        if (newTime % 60 === 0) {
          // Log every minute
          console.log(
            `[QuizTimer] ${Math.floor(newTime / 60)} minutes remaining`,
          );
        }
        return newTime;
      });
  ...
  return (
    <div
      className={`text-lg font-semibold p-2 border rounded-md ${
        isCriticalTime
