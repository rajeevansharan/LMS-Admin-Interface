...
const ShortAnswerManagePage = () => {
  ...
  const handleEdit = (questionId: number) => {
    ...
    const url = `/facultyMember/questions/shortAnswer/modify?questionId=${questionId}${contextParam}`;
    router.push(url);
  };
  ...
  const handleContextChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const newUrl = `/facultyMember/questions/shortAnswer/manage${newContext === "global" ? "" : `?courseId=${newContext}`}`;
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <button
              ...
              onClick={() =>
                selectedContext &&
                router.push(
                  `/facultyMember/questions/shortAnswer/create${selectedContext === "global" ? "" : `?courseId=${selectedContext}`}`,
                )
              }
            >
              Add New Question
        ...
        {isLoading || selectedContext === null ? (
          ...
          <div className="bg-primary p-8 rounded-lg shadow text-center">
            <p className="text-lg mb-4">
              No short answer questions found for this context.
            </p>
            <CustomButton
              label="Create Your First Question"
              url={`/facultyMember/questions/shortAnswer/create${selectedContext === "global" ? "" : `?courseId=${selectedContext}`}`}
              width="w-auto"
            />
          </div>
        ) : (
            ...
            {questions.map((question) => (
              ...
              >
                <div className="mb-4">
                  ...
                  <div className="mt-3">
                    <div className="mb-2 font-medium">Accepted Answer(s):</div>
                    <div className="flex flex-wrap gap-2">
                      <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full">
                        {question.correctAnswerSA || "Not specified"}
                      </span>
                    </div>
                    <div className="mt-2 text-sm">
                      <span
                        className={`${question.caseSensitiveSA ? "text-red-600" : "text-green-600"}`}
                      >
                        {question.caseSensitiveSA
                          ? "Case-sensitive"
                          : "Case-insensitive"}
                      </span>
                    </div>
...
export default ShortAnswerManagePage;
