...
const MultipleChoiceManagePage = () => {
  // --- STATE ---
  ...
  const handleEdit = (questionId: number) => {
    ...
    const url = `/facultyMember/questions/multipleChoice/modify?questionId=${questionId}${contextParam}`;
    router.push(url);
  };
  ...
  const handleContextChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const newUrl = `/facultyMember/questions/multipleChoice/manage${newContext === "global" ? "" : `?courseId=${newContext}`}`;
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <button
              ...
              onClick={() =>
                selectedContext &&
                router.push(
                  `/facultyMember/questions/multipleChoice/create${selectedContext === "global" ? "" : `?courseId=${selectedContext}`}`,
                )
              }
            >
              Add New Question
        ...
        {isLoading || selectedContext === null ? (
          ...
          <div className="bg-primary p-8 rounded-lg shadow text-center">
            <p className="text-lg mb-4">
              No multiple choice questions found for this context.
            </p>
            <CustomButton
              label="Create Your First Question"
              url={`/facultyMember/questions/multipleChoice/create${selectedContext === "global" ? "" : `?courseId=${selectedContext}`}`}
              width="w-auto"
            />
          </div>
        ) : (
              ...
              >
                <div className="mb-4">
                  ...
                  <div className="ml-4 mt-3">
                    {(question.optionsSCMC || []).map((option, index) => (
                      <div
                        key={index}
                        className={`p-2 mb-2 rounded ${
                          (question.correctOptionIndicesMC || []).includes(
                            index,
                          )
...
export default MultipleChoiceManagePage;
