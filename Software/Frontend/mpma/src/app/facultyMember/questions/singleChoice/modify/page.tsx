...
import SingleChoiceQuestionForm, {
  SingleChoiceQuestionFormRef,
  QuestionApiPayload,
} from "../SingleChoiceQuestionForm";
...
const SingleChoiceQuestionModifyPage = () => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const questionId = searchParams.get("questionId");
  const courseId = searchParams.get("courseId");
  const formRef = useRef<SingleChoiceQuestionFormRef>(null);
  ...
  useEffect(() => {
    const fetchQuestionData = async () => {
      ...
      try {
        ...
        // Validate that this is a single choice question
        if (questionData.questionType !== "SINGLE_CHOICE") {
          setError("Invalid question type. Expected single choice question.");
          setIsLoadingQuestion(false);
          return;
        }
          ...
          <div className="bg-primary p-4 md:p-6 rounded-lg shadow-md">
            ...
            <SingleChoiceQuestionForm
              ...
              initialData={{
                ...
                optionIds: initialQuestionData?.optionIds, // Pass option IDs if available
                correctOptionIndex: initialQuestionData?.correctOptionIndexSC,
                difficultyLevel:
...
export default SingleChoiceQuestionModifyPage;
